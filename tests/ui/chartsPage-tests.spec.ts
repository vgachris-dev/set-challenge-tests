...
test.describe("Charts Page (UI E2E)", () => {
  ...
  test("Verify Charts page loads and all elements are visible", async ({
    page,
  }) => {
    // Act
  ...
  test("Verify column headers switch from medium to bold when clicked", async () => {
    ...
    const beforeWeights = await Promise.all(
      chartsPage.columnHeaderButtons.map((h) =>
        HelperFunctions.getFontWeight(h),
      ),
    );
  ...
  test("Verify successful sorting by date created", async ({ page }) => {
    ...
    const dates = HelperFunctions.parseDatesFromRows(
      rows,
      TestConstants.INDEX.SECOND,
    );
  ...
  test("Verify search charts input functionality", async ({ page }) => {
    ...
    const firstChart = (await chartsPage.getChartNames())[
      TestConstants.INDEX.FIRST
    ];
